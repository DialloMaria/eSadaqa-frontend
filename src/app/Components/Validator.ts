...
export class ValidatorData {
  ...
  // Validation de la longueur minimale
  static minLengthValidator(controlName: string, min: number): ValidatorFn {
    return (control: AbstractControl): ValidationErrors | null => {
      const value = control.value || '';
      if (value.length < min) {
        return {
          error: true,
          message: `${controlName} doit contenir au moins ${min} caractères. Vous avez saisi ${value.length} caractères.`,
        };
      }
      return null;
    };
  ...
  static maxLengthValidator(controlName: string, max: number): ValidatorFn {
    return (control: AbstractControl): ValidationErrors | null => {
      const value = control.value || '';
      if (value.length > max) {
        return {
          error: true,
          message: `${controlName} ne doit pas dépasser ${max} caractères. Vous avez saisi ${value.length} caractères.`,
        };
      }
      return null;
    };
  ...
  static requiredValidators(controlName: string): ValidatorFn {
    return (control: AbstractControl): ValidationErrors | null => {
      const value = control.value || '';
      if (!value.trim()) {
        return {
          error: true,
          message: `${controlName} est obligatoire.`,
        };
      }
      return null;
    };
  ...
  static emailValidator(controlName: string) {
    return (control: AbstractControl): ValidationErrors | null => {
      ...
      return !control.value.length
        ? { error: true, message: `L' ${controlName} est obligatoire.` }
        : !control.value.match(new RegExp(format))
        ? { error: true, message: `L' ${controlName} doit respecter le format : yohan@gmail.com.` }
        : null;
    };
  }
  ...
  static passWordValidator(controlName: string, lengthWord:number) {
    return (control: AbstractControl): ValidationErrors | null => {
      ...
      return !control.value.length
        ...
        :((control.value != null) && (control.value.startsWith(' ') && control.value.endsWith(' ')))
          //invalid, return error object
        ?  {  error: true, message: `Ce champs ne peut pas contenir des espaces.` }
        : null;
  ...
  static usernameValidator( controlName: string, minlength: number,maxlength: number) {
    return (control: AbstractControl): ValidationErrors | null => {
...
static dateValidator(controlName: string) {
  return (control: AbstractControl): ValidationErrors | null => {
    ...
    return !control.value.length
      ...
      ? { error: true, message: `${controlName} ne peut pas être une date future.` }
      : null;
  };
}


  allNumberValidator(controlName: string, minLength: number,maxLength: number) {
    let pattern = '^[0-9+]+(\.?[0-9]+)?$';
   ...
   */
  static phoneValidator(controlName: string, minLength: number,maxLength: number) {
    let pattern = '^[0-9+]+(\.?[0-9]+)?$';
    /* Add an extra backslash before \d if regex is constructed as a string. There is no need for // at the beginning and end of string if using RegExp  */
    return (control: AbstractControl): ValidationErrors | null => {
      return !control.value.length
